...
def get_item_warehouse_map(filters: StockBalanceFilter, sle: List[SLEntry]):
    ...
    for d in sle:
        ...
        if d.posting_date < from_date or (
            ...
            qty_dict.opening_val += conversion * value_diff

        elif d.posting_date >= from_date and d.posting_date <= to_date:
            if flt(qty_diff, float_precision) >= 0:
                qty_dict.in_qty += qty_diff
                qty_dict.in_val += conversion * value_diff
            else:
                qty_dict.out_qty += abs(qty_diff)
                qty_dict.out_val += conversion *  abs(value_diff)

        qty_dict.val_rate = conversion * d.valuation_rate
        qty_dict.bal_qty += qty_diff
        qty_dict.bal_val += conversion * value_diff
...
# from frappe import _
# from frappe.query_builder.functions import CombineDatetime
# from frappe.utils import cint, date_diff, flt, getdate, get_currency, get_price
# from frappe.utils.nestedset import get_descendants_of
...
#     exchange_rate = get_price(company_currency, target_currency, filters.get("to_date"))
#     for data_row in data:
#         data_row["bal_val"] *= exchange_rate
#         data_row["opening_val"] *= exchange_rate
#         data_row["in_val"] *= exchange_rate
#         data_row["out_val"] *= exchange_rate
#         data_row["val_rate"] *= exchange_rate
