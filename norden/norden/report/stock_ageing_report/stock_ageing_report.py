...
def get_range_age(filters: Filters, fifo_queue: List, to_date: str, item_dict: Dict) -> Tuple:
    ...
    for item in fifo_queue:
        ...
        if age <= filters.range:
            range1 = flt(range + qty, precision)
...
def get_columns(filters: Filters) -> List[Dict]:
    ...
    columns.extend(
        [
            ...
            {  "label": _("Total Amount"), "fieldname": "amt","fieldtype": "Float",  "width": 100 }
        ]
    )
...
def setup_ageing_columns(filters: Filters, range_columns: List):
    ranges = [
        # f"0 - {filters['range']}",
        # f"{cint(filters['range']) + 1} - {cint(filters['range'])}",

        _("{0} - Above").format(cint(filters.range) + 1),
    ]
