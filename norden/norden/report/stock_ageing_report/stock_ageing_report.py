	...
	for item, item_dict in item_details.items():
		...
		row.extend(
			[
				...
				average_age,
				item_dict.get("total_qty")*valuation_rate,
				range1,
				range1 * valuation_rate,
				range2,
				range2 * valuation_rate,
				range3,
				range3 * valuation_rate,
				range4,
				range4 * valuation_rate,
				range5,
				range5 * valuation_rate,
				range6,
				range6 * valuation_rate,
				above_range6,
				above_range6 * valuation_rate,
				earliest_age,
				latest_age,
				details.stock_uom,
			]
...
def setup_ageing_columns(filters: Filters, range_columns: List):
	ranges = [
		...
		_("{0} - Above").format(cint(filters["range6"]) + 1),
	]
