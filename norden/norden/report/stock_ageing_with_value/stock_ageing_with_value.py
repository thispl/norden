...
def format_report_data(filters: Filters, item_details: Dict, to_date: str) -> List[Dict]:
	...
	for item, item_dict in item_details.items():
		...
		latest_vr = frappe.db.sql("""select valuation_rate as vr from tabBin where item_code = '%s' and warehouse = '%s' """%(details.name,source_warehouse),as_dict=True)

		if len(latest_vr) > 0:
			valuation_rate = latest_vr[0]["vr"]
			val_rate  = round(valuation_rate,2)
		else:
			val_rate = []
			l_vr = frappe.db.sql("""select valuation_rate as vr from tabBin
					where item_code = '%s' """%(details.name),as_dict=True)
			for item in l_vr: 
		...
		tot = item_dict.get("total_qty")*val_rate
		row.extend(
			[
				val_rate,
				item_dict.get("total_qty"),
				average_age,
				tot,
				range1,
				range2,
...
def get_columns(filters: Filters) -> List[Dict]:
	...
	columns.extend(
		[
			...
			{"label": _("Available Qty"), "fieldname": "qty", "fieldtype": "Float", "width": 100,"precision":2},
			{"label": _("Average Age"), "fieldname": "average_age", "fieldtype": "Float", "width": 100,"precision":2},
