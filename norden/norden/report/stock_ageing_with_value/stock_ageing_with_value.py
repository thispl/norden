...
def format_report_data(filters: Filters, item_details: dict, to_date: str) -> list[dict]:
	...
	for _item, item_dict in item_details.items():
		...
		# Get stock value from latest SLE per item+warehouse (same as Stock Balance)
		# Stock Balance uses: bal_qty * val_rate where val_rate is last SLE valuation_rate per wh
		# Use posting_date + posting_time + creation ordering (not MAX(name) which isn't chronological)
		wh_values = frappe.db.sql("""
			SELECT SUM(s.qty_after_transaction * s.valuation_rate) as total_value, SUM(s.qty_after_transaction) as total_qty
			FROM `tabStock Ledger Entry` s
			INNER JOIN (
				SELECT item_code, warehouse, MAX(CONCAT(posting_date, ' ', posting_time, '.', creation)) as max_dt
				FROM `tabStock Ledger Entry`
				WHERE company = '%s' AND posting_date <= '%s' AND docstatus < 2 AND is_cancelled = 0
				GROUP BY item_code, warehouse
			) m ON CONCAT(s.posting_date, ' ', s.posting_time, '.', s.creation) = m.max_dt AND s.item_code = m.item_code AND s.warehouse = m.warehouse
			WHERE s.company = '%s' AND s.item_code = '%s' AND s.qty_after_transaction > 0
		""" % (filters.get("company"), filters.get("to_date"), filters.get("company"), details.name), as_dict=True)
		
		if wh_values and wh_values[0].get("total_value") is not None:
			tot = round(wh_values[0]["total_value"], 2)
			wh_qty = wh_values[0].get("total_qty") or 0
			if wh_qty and wh_qty != 0:
				val_rate = round(tot / wh_qty, 2)
		...
		if val_rate and val_rate>0:
			range1_amount=range1*val_rate
			range2_amount=range2*val_rate
			range3_amount=range3*val_rate
			above_range3_amount=above_range3*val_rate
		else:
			tot=0
			range1_amount=0
			range2_amount=0
			range3_amount=0
			above_range3_amount=0
...
def get_columns(filters: Filters) -> list[dict]:
	...
	columns.extend(
		[	
			{"label": _("Valuation Rate"),"fieldname": "valuation_rate","fieldtype": "Float", "width": 100,"precision":2 },
			...
			{"label": _("Total Amount"), "fieldname": "amt","fieldtype": "Float",  "width": 100 ,"precision":2}
			# {"label": _("Currency"), "fieldname": "currency","fieldtype": "Link",  "width": 100 ,"precision":2,"hidden":1,'default':filter}
		]
	)
...
def setup_ageing_columns(filters: Filters, range_columns: list):
	...
	for i, label in enumerate(ranges):
		...
		amount_fieldname = "amount_range" + str(i + 1)
		add_column(range_columns, label=_("Age ({0})").format(label), fieldname=fieldname)
		add_column(range_columns, label=_("Amount ({0})").format(label), fieldname=amount_fieldname)
