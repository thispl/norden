...
class MovementLedgerFilter(TypedDict):
...
def execute(filters: Optional[MovementLedgerFilter] = None):
    return MovementLedgerReport(filters).run(filters)


class MovementLedgerReport(object):
    def __init__(self, filters: Optional[MovementLedgerFilter]) -> None:
    ...
    def prepare_new_data(self,filters):
        ...
        sre_details = self.get_sre_reserved_qty_details(filters)
        ...
        for key, report_data in self.item_warehouse_map.items():
            ...
            value = report_data.bal_qty - report_data.reserved_stock
            
            report_data.update(
                {"unreserved_stock":value}
            )
            report_data.update(
            	{"bal_val": report_data.unreserved_stock * report_data.val_rate}
			)
            if report_data.bal_qty == 0:
                continue
            else:
                report_data.update(
                    {"total_val": report_data.bal_qty * report_data.val_rate}
                )
    ...
    def get_sre_reserved_qty_details(self,filters) -> dict:
        from erpnext.stock.doctype.stock_reservation_entry.stock_reservation_entry import (
            get_sre_reserved_qty_for_items_and_warehouses_with_date as get_reserved_qty_details,
        )
        ...
        return get_reserved_qty_details(item_code_list, warehouse_list,filters.from_date,filters.to_date)
    ...
    def prepare_stock_ledger_entries(self):
        ...
        query = (
            ...
            .where((sle.docstatus < 2) & (sle.is_cancelled == 0) & (sle.warehouse != 'Stores - NCPL')) 
    ...
    def get_columns(self):
        ...
        columns.extend(
            [
                ...
                # {
                #     "label": _("Total Qty"),
                #     # "label": _("Balance Qty"), ## renamed to Total Qty
                #     "fieldname": "bal_qty",
                #     "fieldtype": "Float",
                #     "width": 100,
                #     "convertible": "qty",
