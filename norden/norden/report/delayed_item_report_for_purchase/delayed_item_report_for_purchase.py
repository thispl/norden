...
class DelayedItemReport(object):
	...
	def get_data(self, consolidated=False):
		...
		purchase_order_field = "purchase_order" if doctype == "Purchase Invoice" else "against_purchase_order"
		...
		query = (
			...
			.select(
				...
				child.amount,
				child.so_detail,
				child[purchase_order_field].as_("purchase_order"),
			...
			.where(
				...
				& (child[purchase_order_field].notnull())
				& (child[purchase_order_field] != "")
			)
		)
		...
		if self.filters.get("purchase_order"):
			query = query.where(child[purchase_order_field] == self.filters.get("purchase_order"))
	...
	def filter_transactions_data(self, consolidated=False):
		purchase_orders = [d.purchase_order for d in self.transactions]
		doctype = "purchase Order"
		filters = {"name": ("in", purchase_orders)}

		if not consolidated:
			purchase_order_items = [d.so_detail for d in self.transactions]
			doctype = "purchase Order Item"
			filters = {"parent": ("in", purchase_orders), "name": ("in", purchase_order_items)}
		...
		for row in self.transactions:
			key = row.purchase_order if consolidated else (row.purchase_order, row.so_detail)
			row.update(
				{
