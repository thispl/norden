...
class SalarySummaryReport(Document):
    pass
...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    ncpl_staff_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          ...
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NCPL STAFF' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    ncpl_pro_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NCPL PROFESSIONAL FEE' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    ncpl_lease_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NCPL LEASE RENT' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    ncpl_intern_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NCPL INTERN' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    
    overall_ncpl_hold = sum([ncpl_staff_hold['hold'] or 0.0,ncpl_pro_hold['hold'] or 0.0,ncpl_lease_hold['hold'] or 0.0,ncpl_intern_hold['hold'] or 0.0])
     
    nric_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NRIC Staff' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]

    marcom_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'Marcom Staff' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    pro_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'R & D / TECHNICAL / PRODUCT' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    sea_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NRIC-SEA' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    uk_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NRIC-UK' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    africa_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NRIC-Africa' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
 
    free_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NRIC Freelance' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
    
    intern_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NRIC Intern' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
    
    cont_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = 'NRIC Contract' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]

    empty_hold = frappe.db.sql(""" SELECT SUM(`tabSalary Detail`.amount) AS hold 
                                          FROM `tabSalary Slip` INNER JOIN `tabEmployee` ON `tabSalary Slip`.employee = `tabEmployee`.name 
                                          INNER JOIN `tabSalary Detail` ON `tabSalary Slip`.name = `tabSalary Detail`.parent 
                                          WHERE  `tabEmployee`.status = 'Active' AND 
                                          `tabEmployee`.payroll_category = ' ' AND 
                                          `tabSalary Slip`.start_date BETWEEN '%s' AND '%s'   
                                          AND `tabSalary Detail`.salary_component = 'Salary Hold'  """
                                          %(args.from_date,args.to_date),as_dict=True)[0]
    
    overall_nric_hold = sum([nric_hold['hold'] or 0.0,marcom_hold['hold'] or 0.0,pro_hold['hold'] or 0.0,sea_hold['hold'] or 0.0,uk_hold['hold'] or 0.0,africa_hold['hold'] or 0.0,free_hold['hold'] or 0.0,intern_hold['hold'] or 0.0,cont_hold['hold'] or 0.0])
    
    overall_hold = sum([overall_ncpl_hold or 0.0,overall_nric_hold or 0.0,empty_hold['hold'] or 0.0])
    ...
    ws.merge_cells(start_row=2,start_column=12,end_row=2,end_column=21)
    ...
    for header in ws.iter_rows(min_row=2, max_row=2, min_col=1, max_col=22):
            for cell in header:
                cell.fill = PatternFill(fgColor='F5B7B1', fill_type = "solid")

    for header in ws.iter_rows(min_row=8, max_row=8, min_col=1, max_col=22):
            for cell in header:
                cell.fill = PatternFill(fgColor='D7BDE2', fill_type = "solid")

    for header in ws.iter_rows(min_row=18, max_row=18, min_col=1, max_col=22):
            for cell in header:
                cell.fill = PatternFill(fgColor='85C1E9', fill_type = "solid")            

    for header in ws.iter_rows(min_row=19, max_row=19, min_col=1, max_col=22):
            for cell in header:
                cell.fill = PatternFill(fgColor='85C1E9', fill_type = "solid")

    for header in ws.iter_rows(min_row=20, max_row=20, min_col=1, max_col=22):
            for cell in header:
                cell.fill = PatternFill(fgColor='D7BDE2', fill_type = "solid")

    for header in ws.iter_rows(min_row=22, max_row=22, min_col=1, max_col=22):
            for cell in header:
                cell.fill = PatternFill(fgColor='D7BDE2', fill_type = "solid")
    ...
    for rows in ws.iter_rows(min_row=1, max_row=22, min_col=1, max_col=22):
        for cell in rows:
            cell.border = border
