...
def margin(item_details,company,currency,margin_currency,exchange_rate,user,price_list,territory,line_item_addition,line_item_discount,footer_discount):
    ...
    if "CFO" in frappe.get_roles(frappe.session.user) or "COO" in frappe.get_roles(frappe.session.user) or "HOD" in frappe.get_roles(frappe.session.user) or "Accounts User" in frappe.get_roles(frappe.session.user):
        ...
        for i in item_details:
            if i["special_cost"] > 0:
                spl = spl + 1
        if spl == 0:
            data+='<tr style="background-color:lightgrey"><td width="150px"><b>ITEM</b></td><td width="400px;"><b>ITEM NAME</b></td><td><b>QTY</b></td><td><b>COST</b></td><td><b>COST %</b></td>'
    ...
    for i in item_details:
        ...
        if "CFO" in frappe.get_roles(frappe.session.user) or "COO" in frappe.get_roles(frappe.session.user) or "HOD" in frappe.get_roles(frappe.session.user) or "Accounts User" in frappe.get_roles(frappe.session.user):
            if i["special_cost"] > 0:
                ...
                data+='<tr><td  align = "right" >%s</td><td  align = "right" >%s</td><td  align = "right" >%s</td><td  align = "right" >%s</td><td  align = "right" >%s</td><'%(i["item_code"],i["description"],i["qty"],round(sbus,2), sbu_margin)
                if price_list in ["Singapore Internal Cost","Bangladesh Internal Cost","Srilanka Internal Cost","Cambodia Internal Cost","Vietnam Internal Cost","Indonesia Internal Cost","Malaysia Internal Cost","Philippines Internal Cost"]:
                ...
                if price_list == "India Landing":
                    data+='<td  align = "right" >%s</td><td  align = "right" >%s</td>'%(round(india_landing,2),round(india_landing_margin,2))
                if price_list == "India LTP":
                    data+='<td  align = "right" >%s</td><td  align = "right" >%s</td>'%(round(india_ltp,2),round(india_ltp_margin,2))
                if price_list == "India SPC":
                    data+='<td  align = "right" >%s</td><td  align = "right" >%s</td>'%(round(india_spc,2),round(india_spc_margin,2))
                if price_list == "India DTP":
                    data+='<td  align = "right" >%s</td><td  align = "right" >%s</td>'%(round(india_dtp,2),round(india_dtp_margin,2))
                if price_list == "India STP":
                    data+='<td  align = "right" >%s</td><td  align = "right" >%s</td>'%(round(india_stp,2),round(india_stp_margin,2))
                if price_list == "India MOP":
                    data+='<td  align = "right" >%s</td><td  align = "right" >%s</td>'%(round(india_mop,2),round(india_mop_margin,2))
                if price_list == "India MRP":
                    data+='<td  align = "right" >%s</td><td  align = "right" >%s</td>'%(round(india_mrp,2),round(india_mrp_margin,2))
    ...
    if "CFO" in frappe.get_roles(frappe.session.user) or "COO" in frappe.get_roles(frappe.session.user) or "HOD" in frappe.get_roles(frappe.session.user) or "Accounts User" in frappe.get_roles(frappe.session.user):
        if spcl == 0:
            data += '<tr><th><b>TOTAL MARGIN </b></th><td  align = "right" ><b>%s<b></td><td align = "right"><b>%s</b></td><td align = "right"><b>%s</b></td><td align = "right"><b>%s</b></td>'%('','',sbu_total,sbu_total_margin)            
            if price_list in ["Singapore Internal Cost","Bangladesh Internal Cost","Srilanka Internal Cost","Cambodia Internal Cost","Vietnam Internal Cost","Indonesia Internal Cost","Malaysia Internal Cost","Philippines Internal Cost"]:
